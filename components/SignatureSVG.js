const SignatureSVG = ({ type, count, accidentalCoords, position, index }) => {
  return (
    <div className="signature">
      <div>
        {count}
        {type}
        {JSON.stringify(accidentalCoords)}
        {JSON.stringify(position)}
      </div>{" "}
      {accidentalCoords.map((sig) => {
        console.log(index, sig);
        return <div>{JSON.stringify(sig)}</div>;
      })}
      <svg
        width="208px"
        height="180px"
